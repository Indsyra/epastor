...
- **Output** : lignes insérées dans la table `videos` (au lieu du JSONL
  ...
  sur la `channel` traitée, `source_tab` renseigné et
  `is_excluded_by_config` réévalué selon `channel.target_tabs`
- **Process** : lire les `channels` d'un pasteur depuis la base (plus
  depuis `config.py`) → pour chaque chaîne, scanner uniquement les
  onglets listés dans `channel.target_tabs` (au lieu de la constante
  fixe `TARGET_TABS` de la v1) → mapper chaque résultat vers le modèle
  `Video` avec son `source_tab` → insérer en base avec `channel_id`/`pastor_id`
  → pour les vidéos déjà en base dont le `source_tab` n'est plus dans
  `target_tabs`, passer `is_excluded_by_config=true` (jamais de
  suppression — voir `DATA_MODEL.md` §3) ; pour celles dont l'onglet est
  redevenu actif, repasser le flag à `false`
...
- **Process** : boucler sur les `videos` avec `speaker_match=true` et
  ...
  puis fallback `en`) → si succès, stocker les segments et renseigner
  `videos.language` avec la langue qui a effectivement réussi (pas une
  supposition depuis le titre) ; si échec, passer `transcript_status=unavailable`
  ou `error`
...
- **Directs encore en cours** : `--extract_flat` (US-03) ne permet pas de
  distinguer fiablement un direct terminé d'un direct encore en cours au
  moment de la découverte (info non exposée en mode léger). Décision :
  ne pas complexifier US-03 pour filtrer ça — un direct encore en cours
  échouera naturellement ici (pas de transcript disponible) et sera
  marqué `unavailable`, sans traitement spécial nécessaire.
...
- **Technos** : LangGraph, OpenAI `gpt-4o-mini` (choix pragmatique — clé
  déjà disponible, rapide/économique pour la phase de test), FAISS
  (retrieval)
